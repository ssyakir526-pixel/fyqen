...
void main() {
  ...
  test('delegates liability removal with aggregate ID normalization', () {
    ...
    final Portfolio original = Portfolio(id: 'portfolio-1', name: 'Main', assets: const [], liabilities: <Liability>[liability], createdAt: DateTime.utc(2026), updatedAt: DateTime.utc(2026));
    final Portfolio updated = const RemoveLiabilityFromPortfolioUseCase()(portfolio: original, liabilityId: ' Liability-1 ', updatedAt: DateTime.utc(2026));

    expect(updated.liabilities, isEmpty);
    expect(original.liabilities, <Liability>[liability]);
    expect(() => const RemoveLiabilityFromPortfolioUseCase()(portfolio: original, liabilityId: 'liability-1', updatedAt: DateTime.utc(2026)), throwsArgumentError);
    expect(() => const RemoveLiabilityFromPortfolioUseCase()(portfolio: original, liabilityId: '   ', updatedAt: DateTime.utc(2026)), throwsArgumentError);
  });
}
